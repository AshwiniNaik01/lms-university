...
const getValidPoints = (points = []) =>
  points.filter((p) => isValidText(p.title) || isValidText(p.description));

const getValidChapters = (chapters = []) =>
  chapters.filter(
    (c) =>
      isValidText(c.title) ||
      getValidPoints(c.points).length > 0 ||
      c.lectures?.length > 0 ||
      c.assignments?.length > 0
  );

const getValidWeeks = (weeks = []) =>
  weeks.filter(
    (w) => isValidText(w.title) || getValidChapters(w.chapters).length > 0
  );

const getValidPhases = (phases = []) =>
  phases.filter(
...
// Filtered phases before rendering
...
//                         getValidWeeks(phase.weeks).map((week) => (
//                         <div
//                           key={week._id}
...
//                                   {week.chapters?.length || 0} Course Content
//                                 </p>
//                               </div>
//                             </div>
...
//                                       className="bg-amber-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-amber-600 transition-all flex items-center gap-2"
//                                     >
//                                       📚 Add Course Content to this Module
//                                     </button>
//                                      )}
//                                   </div>
//                                 </>
...
//                                     No course contents yet in this module.
//                                   </p>
//                                    {canPerformAction(rolePermissions, "curriculum", "create") && (
//                                   <button
//                                     onClick={() =>
...
//           </h4>
//           <p className="text-sm text-gray-600">
//             {getValidChapters(week.chapters).length || 0} Course Content
//           </p>
//         </div>
//       </div>
//       <div className="flex items-center gap-2">
...
//           <>
//             {getValidChapters(week.chapters).map((chapter, index) => (
//               <div
//                 key={chapter._id}
//                 className="border-2 border-gray-200 rounded-lg p-4 bg-gradient-to-r from-gray-50 to-white hover:from-blue-50 transition-all"
//               >
...
//                     <div className="flex-1">
//                       <h5 className="font-semibold text-gray-800 mb-2">
//                         {chapter.title || `Course Content ${index + 1}`}
//                       </h5>
//                       <div className="space-y-2">
//                         {getValidPoints(chapter.points).map((point, pointIndex) => (
//                           <div
...
//                               {point.description && (
//                                 <p className="text-gray-600 text-xs mt-1">{point.description}</p>
//                               )}
//                             </div>
//                           </div>
//                         ))}
...
//               <button
//                 onClick={() =>
//                   navigate(`/add-curriculum?type=chapter&weekId=${week._id}&courseId=${selectedCourse}`)
//                 }
//                 className="bg-amber-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-amber-600 transition-all flex items-center gap-2"
//               >
//                 📚 Add Course Content to this Module
...
//           <div className="text-center py-8">
//             <div className="text-4xl mb-3">📚</div>
//             <p className="text-gray-500 mb-4">No course contents yet in this module.</p>
//              {canPerformAction(rolePermissions, "curriculum", "create") && (
//             <button
//               onClick={() =>
//                 navigate(`/add-curriculum?type=chapter&weekId=${week._id}&courseId=${selectedCourse}`)
...
//               This {COURSE_NAME} doesn't have any curriculum yet.
//             </p>
//              {canPerformAction(rolePermissions, "curriculum", "create") && (
//             <button
//               onClick={() =>
...
//               Choose a {COURSE_NAME} from the dropdown above to manage its
//               curriculum.
//             </p>
//           </div>
//         )}
        ...
        <div className="space-y-6">
          {validPhases.map((phase, index) => (
            ...
            >
              ...
              {/* Weeks Section */}
              {openPhase === phase._id && getValidWeeks(phase.weeks).length > 0 && (
                <div className="p-6 bg-gray-50 space-y-4">
                  {getValidWeeks(phase.weeks).map((week) => (
                    <div
                      key={week._id}
                  ...
                  <div className="flex justify-center pt-4">
                    {canPerformAction(rolePermissions, "curriculum", "create") && (
                      <button
                        onClick={() =>
                          navigate(`/add-curriculum?type=week&phaseId=${phase._id}&courseId=${selectedCourse}`)
                        }
                        className="bg-green-500 text-white px-6 py-3 rounded-xl font-semibold hover:bg-green-600 transition-all flex items-center gap-2"
                      >
                        ➕ Add Module to this Schedule
