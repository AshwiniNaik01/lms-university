...
// import { setRole } from "../../features/auth/roleSlice"; // ✅ FIXED: Correct path
...
export default function LoginForm() {
  ...
  const role = useSelector((state) => state.role.selectedRole); // ✅ get role from redux
  const { login } = useAuth();
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const result = await login(email, password, role); // ✅ send role to login if needed
  ...
  return (
    // <div className="min-h-screen bg-gradient-to-br from-[#e0ecfc] via-[#f5f7fa] to-[#e2e2e2] flex items-center justify-center px-4">
    <div className="fixed top-0 left-0 w-full h-screen bg-gradient-to-br from-[#e0ecfc] via-[#f5f7fa] to-[#e2e2e2] flex items-center justify-center overflow-hidden px-4">
        ...
        <div className="md:w-1/2 bg-white px-10 py-12">
          ...
          <form onSubmit={handleLogin} className="space-y-6">
            ...
            <div>
              ...
              <select
                value={role}
                onChange={(e) => dispatch(setRole(e.target.value))} // ✅ update redux role
                className="w-full px-4 py-2 border border-gray-300 rounded-xl focus:ring-2 focus:ring-blue-300 focus:outline-none transition-all"
                required
              >
