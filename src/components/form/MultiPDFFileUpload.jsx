...
const MultiPDFFileUpload = ({ name, formik, label, multiple = true }) => {
  ...
  // Sync displayFiles with Formik values or inputKey
  useEffect(() => {
    const files = formik.values[name] || [];
    setDisplayFiles(files);
  }, [formik.values[name], name, inputKey]); // important: include inputKey

  // Reset file input when Formik resets
  useEffect(() => {
    const unsubscribe = formik.registerField(name, {
      ...
      onReset: () => {
        ...
        formik.setFieldValue(name, []); // explicitly clear Formik value
      },
    });
      ...
      <div className="relative w-full">
        ...
        <div className="flex items-center justify-between border-2 border-dashed border-gray-300 bg-white px-4 py-3 rounded-lg shadow-sm hover:border-blue-400 transition-all duration-300 z-10">
          <div className="flex items-center space-x-3">
            <FaUpload className="text-blue-600" />
      <span className="text-gray-700 font-medium truncate max-w-[300px]">
  {multiple ? "Choose PDF/DOC files..." : "Choose a PDF/DOC file..."}
</span>
...
//  * MultiPDFFileUpload
//  *
//  * A reusable Formik-compatible file upload component for PDF/DOC/DOCX files.
//  *
//  * Props:
...
//   // Initialize field only if undefined
//   useEffect(() => {
//     if (!formik.values[name]) {
//       formik.setFieldValue(name, []);
//     }
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [name]); // Safe: only runs when 'name' changes
