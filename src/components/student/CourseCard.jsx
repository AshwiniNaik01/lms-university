...
const CourseCard = ({ course, onEnrollSuccess, isEnrolled }) => {
  ...
  //   }
  //   if (
  //     window.confirm(`Are you sure you want to enroll in "${course.title}"?`)
  //   ) {
  //     try {
  ...
  //     } catch (error) {
  //       alert(
  //         `Error enrolling: ${error.response?.data?.message || error.message}`
  //       );
  //     }
  //   }
  // };
...
const handleEnroll = async () => {
  ...
  if (typeof window !== "undefined" && window.confirm(`Are you sure you want to enroll in "${course.title}"?`)) {
    try {
      const response = await enrollInCourseApi(course._id, token);
      ...
      <div className="card-actions">
        ...
        {isAuthenticated &&
          ...
          (isEnrolled ? (
            ...
            <button onClick={handleEnroll} className="button button-primary">
              Enroll
            </button>
